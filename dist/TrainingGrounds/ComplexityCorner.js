 ...
 * Time: O(1), Space: O(1)
 */
export function getFirstAndLast(arr) {
 ...
 */
export function findAllPairs(arr) {
    const pairs = [];
    for (let i = 0; i < arr.length; i++) {
 ...
 * Time: O(n²), Space: O(1)
 */
export function hasDuplicatesNaive(arr) {
    for (let i = 0; i < arr.length; i++) {
        for (let j = i + 1; j < arr.length; j++) {
            if (arr[i] === arr[j]) {
                return true;
 ...
 */
export function hasDuplicatesEfficient(arr) {
    const seen = new Set();
 ...
 */
export function findZeroSumTriplets(arr) {
    const triplets = [];
    const n = arr.length;
    for (let i = 0; i < n - 2; i++) {
 ...
 */
export function fibonacciMemoized(n, memo = new Map()) {
    if (n <= 1)
        return n;
    if (memo.has(n))
        return memo.get(n);
 ...
 * Time: O(n), Space: O(1)
 */
export function sumIterative(arr) {
    let total = 0;
    for (const num of arr) {
        total += num;
    }
 ...
 * Time: O(n²), Space: O(n²)
 */
export function create2DArray(n) {
    const grid = [];
    for (let i = 0; i < n; i++) {
        const row = [];
        for (let j = 0; j < n; j++) {
 ...
 */
export function buildFrequencyMap(arr) {
 ...
 * Time: O(n²), Space: O(1)
 */
export function twoSumNaive(arr, target) {
    for (let i = 0; i < arr.length; i++) {
        for (let j = i + 1; j < arr.length; j++) {
 ...
 */
export function twoSumOptimized(arr, target) {
    const seen = new Map();
    for (let i = 0; i < arr.length; i++) {
 ...
 */
export function comparePerformance(implementations) {
...
//# sourceMappingURL=ComplexityCorner.js.map